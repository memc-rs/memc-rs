...
mod tests {
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn flush_quiet_should_remove_all(handler: BinaryHandler) {
        let value = from_string("test value");
        for key_suffix in 0..100 {
            let key = Bytes::from(String::from("test_key") + &key_suffix.to_string());
            insert_value(&handler, key.clone(), value.clone());
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn add_request_should_succeed_if_item_not_exists_and_fail_if_exists(handler: BinaryHandler) {
        let key = Bytes::from("key");
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn add_quiet_request_should_succeed_if_item_does_not_exists(handler: BinaryHandler) {
        let key = Bytes::from("key");
        let mut header = create_header(network::Command::Add, &key);
        const FLAGS: u32 = 0xDEAD_BEEF;
        let value = from_string("value");
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn replace_request_should_fail_if_not_exists_and_succeed_if_exists(handler: BinaryHandler) {
        let key = Bytes::from("key");
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn replace_quiet_request_should_fail_if_not_exists_and_succeed_if_exists(handler: BinaryHandler) {
        let key = Bytes::from("key");
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn append_request_should_succeed_when_value_exists(handler: BinaryHandler) {
        let key = Bytes::from("hello");
        let value = from_string("hello ");
        insert_value(&handler, key.clone(), value.clone());

        let header = create_header(network::Command::Append, &key);
        let request = decoder::BinaryRequest::Append(network::AppendRequest { header, key: key.clone(), value: from_string("world!") });
        let result = handler.handle_request(request);
        match result {
            Some(resp) => {
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn append_quiet_request_should_succeed_when_value_exists(handler: BinaryHandler) {
        let key = Bytes::from("hello");
        let value = from_string("hello ");
        insert_value(&handler, key.clone(), value.clone());

        let header = create_header(network::Command::AppendQuiet, &key);
        let request = decoder::BinaryRequest::AppendQuietly(network::AppendRequest { header, key: key.clone(), value: from_string("world!") });
        let result = handler.handle_request(request);
        match result {
            Some(_resp) => unreachable!(),
            None => {}
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn prepend_request_should_succeed_when_value_exists(handler: BinaryHandler) {
        let key = Bytes::from("hello");
        let value = from_string(" hello");
        insert_value(&handler, key.clone(), value.clone());

        let header = create_header(network::Command::Prepend, &key);
        let request = decoder::BinaryRequest::Prepend(network::PrependRequest { header, key: key.clone(), value: from_string("world!") });
        let result = handler.handle_request(request);
        match result {
            Some(resp) => {
    ...
    #[test_case(create_moka_handler() ; "moka_backend")]
    #[test_case(create_dash_map_handler() ; "dash_map_backend")]
    fn prepend_quiet_request_should_succeed_when_value_exists(handler: BinaryHandler) {
        let key = Bytes::from("hello");
        let value = from_string("hello");
        insert_value(&handler, key.clone(), value.clone());

        let header = create_header(network::Command::PrependQuiet, &key);
        let request = decoder::BinaryRequest::PrependQuietly(network::PrependRequest { header, key: key.clone(), value: from_string("world! ") });
        let result = handler.handle_request(request);
        match result {
            Some(_resp) => unreachable!(),
            None => {}
